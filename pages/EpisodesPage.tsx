            ...
            <div className="sticky top-24">
                <div className="bg-dark p-6 rounded-2xl">
                    ...
                    <div className="border-b border-darker pb-6 mb-6">
                        ...
                        {GRADES.map(grade => (
                            <div key={grade.name}>
                                <button onClick={() => setOpenGrade(openGrade === grade.name ? null : grade.name)} className="w-full flex justify-between items-center py-2 text-light">
                                    <span>{grade.name}</span>
                                    <ChevronDown className={`transition-transform ${openGrade === grade.name ? 'rotate-180' : ''}`} size={20} />
                                </button>
                         ...
                         <div className="space-y-3">
                            {CATEGORIES.map(cat => (
                                <label key={cat.name} className="flex items-center gap-3 text-light/80 cursor-pointer">
                                    <input type="checkbox" className="w-5 h-5 rounded bg-darker border-light/30 text-primary focus:ring-primary"/>
